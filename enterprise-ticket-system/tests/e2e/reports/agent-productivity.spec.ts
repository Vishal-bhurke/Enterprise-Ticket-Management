...
test.describe('Agent Productivity Report', () => {
  test('agent productivity report loads for admin', async ({ page }) => {
  ...
  test('shows report content or empty state', async ({ page }) => {
    await loginAs(page, 'admin');
    await page.goto('/reports/agents');
    await page.waitForLoadState('networkidle');
    const hasContent =
      (await page.locator('p-table, [class*="metric"], [class*="chart"], canvas').first().isVisible()) ||
      (await page.locator('app-empty-state').isVisible());
    expect(hasContent).toBe(true);
  });

  test('agent is denied agent productivity report', async ({ page }) => {
