...
test.describe('Priorities Master', () => {
  test.beforeEach(async ({ page }) => {
    await loginAs(page, 'admin');
    await page.goto('/masters/priorities');
    await page.waitForLoadState('networkidle');
  });

  test('priorities list page loads', async ({ page }) => {
  ...
  test('create priority', async ({ page }) => {
    ...
    const uniqueName = `Priority E2E ${Date.now()}`;
    ...
    await cleanupRow('priorities', { name: uniqueName });
  });
